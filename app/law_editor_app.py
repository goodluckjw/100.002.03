...
st.markdown("<h1 style='font-size:20px;'>📘 부칙개정 도우미 (100.002.03)</h1>", unsafe_allow_html=True)
...
with st.expander("ℹ️ 사용법 안내"):
    st.markdown(      
        ...
        "     - 공백이 포함된 문자열을 개정하려는 경우에는 문자열 전체를 큰따옴표로 감싸서 입력주세요. (예. \"특정범죄 가중처벌 등에 관한 법률\") \n" 
        "- 이 앱은 현행 법률의 본문만을 검색 대상으로 합니다. 헌법, 폐지법률, 시행령, 시행규칙, 행정규칙, 제목, 부칙 등은 검색하지 않습니다. \n"
        "- 이 앱은 업무망에서는 작동하지 않습니다. 인터넷망에서 사용해주세요. \n"
        "- 가운뎃점(U+318D, 법제처 사이트에서 사용)과 중간점(U+00B7, 주로 한글 프로그램과 국회 법령정보스시템에서 사용) 중 어느 것으로 입력하여도 기능하도록 하였습니다. 출력시에는 가운뎃점으로 표기됩니다. \n"
